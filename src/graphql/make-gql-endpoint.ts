...
import { GraphQLEndpoint, GraphQLEndpointArgs, GraphQLModule, GraphQLPerRequestCache } from './types'
...
const bindPerRequestCache = <TContext>(ctx: TContext): GraphQLPerRequestCache => {
	const c = ctx as any
	// fast return
...
export function makeGqlEndpoint<TContext = any, TModel = any>({ schema, resolvers }: GraphQLModule<TContext, TModel>): GraphQLEndpoint<TContext, TModel> {
	const es = makeExecutableSchema({
		...
		resolvers: mapValues(
			resolvers,
			entity => mapValues(entity, field => async (source, args, ctx, info) => field({ source, ctx, args, info, cache: bindPerRequestCache<TContext>(ctx) }))),
	})
