...
import { swaggerModule, SwaggerModuleProps } from '..'
import { createApplication } from '../../../application'

const createTestApp = (props?: SwaggerModuleProps) => createApplication({
...
describe('swagger-module, routePrefix=/api/v1/test', () => {
	it('GET /api/v1/test/swagger.json responds with JSON', async () => createTestApp({ routePrefix: '/api/v1/test' }).run(
		async server => {
			const { headers, status } = await request(server)
	...
	it('GET / redirects to /api/v1/test', async () => createTestApp({ routePrefix: '/api/v1/test' }).run(
		async server => {
			const { status, headers } = await request(server)
				.get('/')
			expect(status).toBe(StatusCodes.MOVED_TEMPORARILY)
	...
	it('GET /api/v1/test/swagger responds with HTML', async () => createTestApp({ routePrefix: '/api/v1/test' }).run(
		async server => {
			const { status, headers } = await request(server)
				.get('/api/v1/test')
			expect(status).toBe(StatusCodes.OK)
